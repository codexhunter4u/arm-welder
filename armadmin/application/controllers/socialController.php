...
class SocialController extends MY_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('SocialModel', 'model');
    }
    ...
    */
    ...
    public function index() {
        ...
        $this->load->view('v_social.php', $data);
    ...
    */
    ...
    public function addRow($rowId = null) {
        ...
        $config = IMG_UPLOAD;
        $config['upload_path'] = $config['upload_path'].'/social/';
        ...
        if($_FILES['upload_img']['size'] > 0){
            if($this->upload->do_upload("upload_img")){
                ...
                $filename = $data['file_path'].'social_'.$unique.'.'.$fileExt;
                ...
                $imagDetails['img_path'] = base_url('/assets/images/social/'.'social_'.$unique.'.'.$fileExt);
                if($result['status']){
                    if($rowId > 0){
    ...
    */
    ...
    public function activateRow() {
        ...
        $formData = array('media_status' => $status);
    ...
    */
    ...
    public function getRows() {
        ...
        foreach ($list as $val) {
            ...
            $row   = array();
            $row[] = $no;
            $row[] = '<img class="img-responsive rowImg" src="'.$val['img_path'].'" alt="'.$val['media_name'].'">';
            $row[] = $val['media_name'];
            $row[] = $val['media_status'] == 1 ? '<b class="non-mandetory">Active<b>' : '<b class="mandetory">In-Active<b>';
            $row[] = ''
                . '<a class="agile-icon" onclick="activateRow('.$val['row_id'].','.$val['media_status'].')" href="#" title="Activate"><i class="fa fa-check-square-o"></i></a>&nbsp;&nbsp;'
    ...
    */
    ...
    public function unLinkImages($rowId) {
        ...
        @unlink('./assets/images/social/'.$imageName[8]);
    }
